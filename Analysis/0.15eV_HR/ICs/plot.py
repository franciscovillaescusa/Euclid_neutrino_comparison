...
fig=figure(figsize=(5,6))     #give dimensions to the figure
gs = gridspec.GridSpec(3,1,height_ratios=[5,2,2])
ax1=plt.subplot(gs[0])
ax2=plt.subplot(gs[1])
...
ax1.set_ylabel(r'$P_{\rm N-GenIC}(k)$',fontsize=18)
ax2.set_ylabel(r'${\rm Ratio}$',fontsize=18)
ax3.set_ylabel(r'${\rm Ratio}$',fontsize=18)


f_out='Pk_ICs.pdf'
f1 = 'Pk_ICs_cb_0.15eV_HR.txt'
f2 = '../../0.15eV/ICs/Pk_ICs_cb_0.15eV.txt'
f3 = 'Pk_ICs_n_0.15eV_HR.txt'
f4 = '../../0.15eV/ICs/Pk_ICs_n_0.15eV.txt'
k1, Pk1, Nmodes = np.loadtxt(f1, unpack=True) 
k2, Pk2, Nmodes = np.loadtxt(f2, unpack=True) 
k3, Pk3, Nmodes = np.loadtxt(f3, unpack=True) 
k4, Pk4, Nmodes = np.loadtxt(f4, unpack=True) 
p1,=ax1.plot(k1,Pk1,linestyle='-',marker='None', c='r')
p2,=ax1.plot(k2,Pk2,linestyle='-',marker='None', c='b')
p3,=ax1.plot(k3,Pk3,linestyle='-',marker='None', c='g')
p4,=ax1.plot(k4,Pk4,linestyle='-',marker='None', c='purple')
p5,=ax2.plot(k1,Pk1/Pk1,linestyle='-',marker='None', c='r')
p6,=ax2.plot(k1,Pk2/Pk1,linestyle='-',marker='None', c='b')
p7,=ax3.plot(k3,Pk3/Pk3,linestyle='-',marker='None', c='green')
p8,=ax3.plot(k3,Pk4/Pk3,linestyle='-',marker='None', c='purple')
#p5,=ax2.plot([np.min(k1), np.max(k1)],[1, 1],linestyle='--',marker='None', c='k')

#place a label in the plot
ax1.text(0.70,0.9, r"$z=127$", fontsize=18, color='k',transform=ax1.transAxes)

#legend
ax1.legend([p1,p2],
           [r"$1024^3$",
            r"$512^3$"],
           loc=3,prop={'size':18},ncol=1,frameon=True)
